...
def ingest_gold_from_api(symbol="GC*1", start_date=None, end_date=None, years=5):
    """
    Ingest Gold data directly from Polygon API.
    Wrapper around universal_ingestion.ingest_from_polygon_api for backward compatibility.
    
    Args:
        symbol: Symbol (default GC*1 which maps to C:XAUUSD for Polygon)
    ...
    """
    return ingest_from_polygon_api(
        api_symbol=symbol,
        asset_class="Commodities",
        start_date=start_date,
        end_date=end_date,
        years=years,
        db_symbol=symbol # Use the same symbol for DB or let it default
    )
